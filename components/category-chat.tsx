...
export function CategoryChat() {
  ...
  function pillClasses(isActive: boolean): string {
    return `rounded-full border px-3.5 py-1.5 text-sm transition-colors ${
      isActive
        ...
        : "border-line bg-card/70 text-ink hover:border-ink/30 hover:bg-white"
    }`;
  }
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {categories.map((category) =>
            confirmDeleteId === category.id ? (
              <div
                key={category.id}
                className="flex items-center gap-1.5 rounded-full border border-line bg-white px-3 py-1.5 font-mono text-[12px]"
              >
              ...
              >
                <button
                  ...
                  className="rounded-full py-1.5 pr-1.5 pl-3.5 text-sm"
                >
                  {category.name}{" "}
          ...
          {creating ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-dashed border-line px-3 py-1.5 text-sm text-ink-muted transition-colors hover:border-ink/30 hover:text-ink"
            >
      ...
      <div className="surface-panel min-h-[420px] p-4 sm:p-5">
        <ChatPanel
          key={scopeKey(scope)}
